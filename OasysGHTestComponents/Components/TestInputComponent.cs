  ...
  public class TestInputComponent : GH_OasysComponent {
    ...
    protected override void SolveInstance(IGH_DataAccess da) {
      if (Params.Input[0].SourceCount > 0) {
        da.SetData(0, Helpers.Input.UnitNumber(this, da, 0, MomentUnit.KilonewtonMeter));
      }
      if (Params.Input[1].SourceCount > 0) {
        da.SetDataList(1, Helpers.Input.UnitNumberList(this, da, 1, ForceUnit.Kilonewton));
      }

      if (Params.Input[2].SourceCount > 0) {
        da.SetData(0, Helpers.Input.GenericGoo<Parameters.GH_UnitNumber>(this, da, 2));
      }
      if (Params.Input[3].SourceCount > 0) {
        da.SetDataList(1, Helpers.Input.GenericGooList<Parameters.GH_UnitNumber>(this, da, 3));
      }

      if (Params.Input[4].SourceCount > 0) {
        da.SetData(0, Helpers.Input.LengthOrRatio(this, da, 4, LengthUnit.Meter));
      }
      if (Params.Input[5].SourceCount > 0) {
        da.SetDataList(1, Helpers.Input.LengthsOrRatios(this, da, 5, LengthUnit.Meter));
      }

      if (Params.Input[6].SourceCount > 0) {
        da.SetData(0, Helpers.Input.UnitNumberOrDoubleAsRatioToPercentage(this, da, 6));
      }
      if (Params.Input[7].SourceCount > 0) {
        da.SetData(0, Helpers.Input.RatioInDecimalFractionToPercentage(this, da, 7));
      }
      if (Params.Input[8].SourceCount > 0) {
        da.SetData(0, Helpers.Input.RatioInDecimalFractionToDecimalFraction(this, da, 8));
      }
    }
  }
}

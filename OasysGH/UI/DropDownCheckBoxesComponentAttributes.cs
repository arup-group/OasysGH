...
using CheckBox = OasysGH.UI.Helpers.CheckBox;
...
namespace OasysGH.UI {
  ...
  public class DropDownCheckBoxesComponentAttributes : GH_ComponentAttributes {
    ...
    private readonly Action<List<bool>> _toggleAction;
    private readonly List<bool> _toggles;
    private readonly List<string> _toggleTexts;
    ...
    private List<RectangleF> _toggleBoxBound;

    // surrounding bounds for toggle boxes
    ...
    public DropDownCheckBoxesComponentAttributes(GH_Component owner, Action<int, int> clickHandle, List<List<string>> dropdownContents, List<string> selections,
      ...
      _toggles = checkBoxInitialStates;
      _toggleTexts = checkBoxNames;
      _toggleAction = toggleHandle;
    }
    ...
    //function sending back the toggle actions

    // the displayed texts for each check box
    ...
    public override GH_ObjectResponse RespondToMouseUp(GH_Canvas sender, GH_CanvasMouseEvent e) {
      ...
      if (e.Button == MouseButtons.Left) {
        ...
        for (int i = 0; i < _toggleTexts.Count; i++) {
          if (_toggleBoxBound[i].Contains(e.CanvasLocation) || _toggleTextBound[i].Contains(e.CanvasLocation)) {
    ...
    // surrounding bounds for toggle box names
    protected override void Layout() {
      ...
      for (int i = 0; i < _dropdownlists.Count; i++) {
        ...
        var tempText = new RectangleF(_borderBound[i].X, _borderBound[i].Y, _borderBound[i].Width - bw, _borderBound[i].Height);
      ...
      // spacer
      int i_after_dropdown = _dropdownlists.Count;
      if (_spacerTxts[i_after_dropdown] != "") {
      ...
      // check boxes
      _toggleTextBound = new List<RectangleF>();
      _toggleBoxBound = new List<RectangleF>();

      for (int i = 0; i < _toggleTexts.Count; i++) {
        // add text box
        _toggleTextBound.Add(new RectangleF(
          Bounds.X + 2 * s + bw, //x
          Bounds.Bottom + s + h0 + i * h1, //y
          Bounds.Width - 2 * s - bw, //width
          h1)); // height
        // add check box
        _toggleBoxBound.Add(new RectangleF(Bounds.X + s, Bounds.Bottom + s + h0 + i * h1, bw, h1));
      }
      // update component bounds
      Bounds = new RectangleF(Bounds.X, Bounds.Y, Bounds.Width, Bounds.Height + h0 + _toggleTexts.Count * h1 + 2 * s);
    }

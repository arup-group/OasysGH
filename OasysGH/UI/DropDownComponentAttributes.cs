...
namespace OasysGH.UI
{
  ...
  public class DropDownComponentAttributes : GH_ComponentAttributes
  {
    ...
    float MinWidth
    {
      get
      {
        float sp = WidthAttributes.MaxTextWidth(spacerTxts, GH_FontServer.Standard);
        float bt = 0;
        for (int i = 0; i < dropdownlists.Count; i++)
        {
          float tbt = WidthAttributes.MaxTextWidth(dropdownlists[i], new Font(GH_FontServer.FamilyStandard, 8));
          if (tbt > bt)
            bt = tbt;
        }

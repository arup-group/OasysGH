...
using Xunit;
using UnitNumber = OasysGH.Parameters.GH_UnitNumber;

namespace OasysGHTests.Components {
  [Collection("GrasshopperFixture collection")]
  public class InputComponentT {
    [Fact]
    public void UnitNumberInputTest() {
      var comp = new TestInputComponent();
      comp.CreateAttributes();

      var quantity = new Moment(15, OasysUnits.Units.MomentUnit.MeganewtonMeter);
      SetInput(comp, quantity, 0);

      var output = (UnitNumber)GetOutput(comp, 0);
      Assert.Equal(15, output.Value.Value);
    }


    private static object GetOutput(
      GH_Component component, int index = 0, int item = 0,
      ...
      return component.Params.Output[index].VolatileData.get_Branch(0)[item];
    }
